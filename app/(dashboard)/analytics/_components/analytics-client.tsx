...
export default function AnalyticsClient({ initialPromos }: { initialPromos: ActivePromo[] }) {
  const [selectedPromo, setSelectedPromo] = useState<string>('all');
  // Données initiales fournies par le serveur (plus de fetch au montage → pas de flash).
  const [promos] = useState<ActivePromo[]>(initialPromos);
