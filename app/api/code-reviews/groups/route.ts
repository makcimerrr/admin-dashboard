...
import {
  getProjectNamesByTrack,
  getTrackByProjectName
      ...
      for (const projectName of projectsToProcess) {
        ...
        for (const group of projectGroups) {
          ...
          if (audit) {
            ...
            validatedCount =
              audit.results?.filter((r) => r.validated).length || 0;
            const totalMembers = audit.results?.length || 0;
            const validationRate =
              totalMembers > 0 ? (validatedCount / totalMembers) * 100 : 100;
