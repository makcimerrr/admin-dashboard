...
function findProjectIndex(projectName: string): number {
    let globalIndex = 0;
    for (const track of Object.keys(allProjectsTyped) as Array<keyof AllProjects>) {
...
function calculateDelayLevel(
    ...
    const currentPromoProject = promoStatus[promotionTitle as keyof typeof promoStatus];
    ...
    if (typeof currentPromoProject === 'string' && currentPromoProject.toLowerCase() === 'fin') {
        ...
        if (promoProjectTrack) {
            ...
            if (studentProjectInTrack?.toLowerCase() === currentPromoProject.toLowerCase()) {
                ...
                const studentIndex = studentProjectInTrack ? findProjectIndex(studentProjectInTrack) : -1;
...
async function updatePromoStudents(eventId: string): Promise<{ updated: number; errors: string[] }> {
    ...
    try {
        ...
        if (!response.ok) {
            return { updated: 0, errors: [`Erreur API Zone01 pour ${eventId}: ${response.status}`] };
        }
        ...
        for (const login of Object.keys(userProjects)) {
            try {
                const { commonProjects, lastProjectsFinished } = findActiveProjectsByTrack(userProjects[login]);
                ...
                const isRustActive = commonProjects['Rust']?.name !== firstRustProject ||
                                     commonProjects['Rust']?.status !== 'without group';
                const isJavaActive = commonProjects['Java']?.name !== firstJavaProject ||
                ...
                const { name: actualProjectName, status: actualProjectStatus } = findLastActiveProject(commonProjects);
                const delayLevel = calculateDelayLevel(promotionTitle, commonProjects, lastProjectsFinished);
                ...
                errors.push(`Erreur pour ${login}: ${err instanceof Error ? err.message : 'Erreur inconnue'}`);
            }
        }
    } catch (err) {
        errors.push(`Erreur globale pour ${eventId}: ${err instanceof Error ? err.message : 'Erreur inconnue'}`);
    }
...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        await fetch(`${request.nextUrl.origin}/api/timeline_project`);
        ...
        const skippedArchived = promotions.length - promoIds.length - (promoId ? promotions.length - 1 : 0);
