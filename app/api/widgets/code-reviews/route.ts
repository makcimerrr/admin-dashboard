...
import {
  ...
  getUniqueStudents,
  getUnauditedStudents,
  Zone01ProgressEntry
} from '@/lib/services/zone01';
import {
  getAuditedStudentsByPromoAndTrack,
  getRecentAudits
...
function filterOutDropouts(
  ...
  return progressions.filter(
    (p) => !dropoutLogins.has(p.user.login.toLowerCase())
  );
}
...
export async function GET() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_URL ?? ''}/api/promotions/active`,
      ...
      try {
        ...
        for (const track of tracks) {
          ...
          const projectNamesLower = new Set(
            (await projectNames).map((n) => n.toLowerCase())
          );

          // Récupérer les étudiants audités pour ce tronc
          const auditedStudentsMap = await getAuditedStudentsByPromoAndTrack(
            promoId,
            track
          );

          // Calculer les étudiants non audités
          const unauditedLogins = getUnauditedStudents(
            progressions,
            await projectNames,
            auditedStudentsMap
          );
