 ...
 */
export async function getAuditById(id: number): Promise<AuditWithResults | undefined> {
 ...
 */
export async function getRecentAuditsWithResults(limit: number = 10): Promise<AuditWithResults[]> {
    return db.query.audits.findMany({
        orderBy: [desc(audits.createdAt)],
        limit,
        with: {
...
export async function getAuditedGroupIds(
    ...
    const result = await db.select({ groupId: audits.groupId })
        .from(audits)
        .where(and(
            eq(audits.promoId, promoId),
            eq(audits.projectName, projectName)
 ...
 */
export async function createAudit(data: CreateAuditInput): Promise<AuditWithResults> {
...
export async function updateAudit(
    ...
    return db.transaction(async (tx) => {
        ...
        const [updatedAudit] = await tx.update(audits)
            .set({
                summary: data.summary,
                warnings: data.warnings,
                updatedAt: new Date(),
 ...
 */
export async function getAuditStatsForPromo(promoId: string): Promise<AuditStats[]> {
    ...
    for (const audit of allAudits) {
        ...
        let projectStat = stats.projectStats.find(p => p.projectName === audit.projectName);
        if (!projectStat) {
            projectStat = {
...
export async function getAuditCountByAuditor(
    ...
    const countMap = new Map<string, { auditorId: number | null; auditorName: string; count: number }>();
